...
func TestPinnedTrailTitleCountsLegsAbove(t *testing.T) {
	...
	if got := m.trailTitle(60); !strings.Contains(got, "↑ ") || !strings.HasSuffix(strings.TrimSpace(got), "legs") {
...
func TestStuckHeadNamesTheHungCall(t *testing.T) {
	...
	m.point(api)
	openTrail(m)
	if got := strings.Join(m.trailColumn(70, 20), "\n"); !strings.Contains(got, "◍▸fix    Bash: python backfill.py --all") || strings.Count(oneSpace(strings.ReplaceAll(got, "▸", " ")), "◍ fix Bash: python backfill.py --all") != 1 || !strings.Contains(got, "silent 4m") {
		t.Errorf("the single trail's HEAD is the hung call, said once (#100, #104):\n%s", got)
	}
}
...
func TestPromptsAreChapters(t *testing.T) {
	...
	if !strings.Contains(rows[2+blockHeight(m.trail)], "fix all the failures first") { // under the block (#377)
...
func TestFoldRowCountsShipsAndRedRuns(t *testing.T) {
	...
	// fold row keeps what is above it and when it began (#377).
...
func TestTrailTitleGoesCompact(t *testing.T) {
	...
	if got := m.trailTitle(44); !strings.Contains(got, "5h") || strings.Contains(got, "…") || strings.Contains(got, "⚑") {
		t.Errorf("a narrow title should keep its span whole and leave the ships to the block (#377): %q", got)
	}
}
...
func TestFoldTallyIsTheWholeDays(t *testing.T) {
	...
	for _, got := range []string{short, tall} {
		if !strings.Contains(got, "◆ ship   4 legs") {
			t.Errorf("the tally is not the whole day's, at either height (#377):\n%s", got)
		}
	}
}
