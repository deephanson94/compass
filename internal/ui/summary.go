...
// The summary (#344). A trail a day long is two hundred rows, and the
...
// it; `tab` on a leg is the trail with the cursor on that leg, the
// conversation beside it as it always is; `s` or `esc` is the trail as it
// was left. It is a view of the legs, not a level: the depth stays at
// three (board, session, summary; a class's legs are the summary's own
// rows, like a leg's waypoints are the trail's).

// summaryRow is one row of the summary: a class with its count, a leg
// under an open class, the lanes' count, or a lane under it.
...
func (m *Model) summaryRefusal() string {
	...
	case m.level == levelBoard || m.level == levelTrail:
		return "the summary is the legs' · tab, then s"
...
// the summary was. Opening it puts the cursor on the first row; closing it
// leaves the trail's cursor where it stood.
...
func (m *Model) summaryKey(key string) bool {
	...
	case " ", "space":
		...
		// puts the cursor on the class it folded into.
	...
	case "tab":
		switch row.kind {
		case "class", "lanes":
			// Deeper into a class is its legs: open it, and stand on the
			// first. A class already open just moves the cursor on.
...
func summaryGroupRow(rows []summaryRow, i int) int {
	for j := i; j >= 0; j-- {
		if rows[j].kind == "class" || rows[j].kind == "lanes" {
			return j
		}
	}
	return 0
...
// class, close an open one — or the group a leg is under.
func (m *Model) summaryFoldWord() string {
	...
	m.summaryClamp(rows)
	if m.summaryOpen[rows[m.summaryCursor].key] {
		return "close"
	}
	return "open"
}
...
// `↑ 12 above` on its first row, `▾ 20 more below` on its last.
func (m *Model) summaryLines(w, h int) []string {
	...
	if tail {
		out = append(out, dimStyle.Render(clip(fmt.Sprintf("▾ %d more below", len(lines)-end), w)))
	}
	return fit(out, h)
}
...
// where the age goes: `◆ build   3 legs · 2✗        1h20m`. A leg under
// it is the row the trail draws for that leg, hung on the trail's own
// waypoint rail so the group reads as one.
func (m *Model) summaryRow(rows []summaryRow, i, w int) string {
	r := rows[i]
	switch r.kind {
	case "class":
		return summaryClassRow(m.trail, r.class, w)
	case "lanes":
		return summaryLanesRow(m.trail, m.now, w)
	case "leg":
		o := m.trailOpts(w-trailWayWidth, 1)
		l := m.trail.Legs[r.leg]
		label, narrated := legLabel(l, o)
		return summaryHang(rows, i) + legRow(l, label, narrated, o)
	case "lane":
		return summaryHang(rows, i) + summaryLaneRow(m.trail.Branches[r.lane], m.now, w-trailWayWidth)
	}
	return ""
}
...
func summaryHang(rows []summaryRow, i int) string {
	mark := wayTee
	if i+1 >= len(rows) || rows[i+1].kind == "class" || rows[i+1].kind == "lanes" {
		mark = wayEnd
	}
	return ruleStyle.Render(railStroke+"  "+mark) + " "
}

// summaryClassRow is `◆ build   3 legs · 2✗        1h20m`: the count, the
// red runs where the class is test, and the time the class took, summed
// over its legs.
func summaryClassRow(tr journey.Trail, c journey.Class, w int) string {
	n, red := 0, 0
	...
	head := classStyle(c).Render(glyphLeg + " " + pad(c.String(), trailVerbWidth))
	count := plural(n, "leg")
	badge := ""
	if red > 0 {
		badge = fmt.Sprintf("%d✗", red)
	}
	age := ""
	if span >= time.Minute {
		age = state.ShortDuration(span)
	}
	return summaryFigureRow(head, count, badge, age, w)
}

// summaryLanesRow is `◈ agent   4 lanes · 1 out`: the lanes sent out, and
// how many are still out.
func summaryLanesRow(tr journey.Trail, now time.Time, w int) string {
	out := 0
	...
	badge := ""
	if out > 0 {
		badge = fmt.Sprintf("%d out", out)
	}
	return summaryFigureRow(head, plural(len(tr.Branches), "lane"), badge, "", w)
}

// summaryFigureRow lays a count row out on the leg row's grid: the head,
// the count, a badge after a dot, and the age at the right edge.
func summaryFigureRow(head, count, badge, age string, w int) string {
	body := w - trailPrefixWidth - 1
	if body < 1 {
		return head
	}
	text := count
	if badge != "" && len([]rune(text))+3+len([]rune(badge)) <= body-len([]rune(age))-1 {
		text += " · " + badge
	}
...
// ✓ 7m ago`, `⋯ 20m out` while it is out, `⌀` back with nothing.
func summaryLaneRow(br journey.Branch, now time.Time, w int) string {
	mark := branchOpen
	tail := mark + " " + relAge(now, br.Start) + " out"
	if br.Done {
		mark = branchDone
		...
		tail = mark + " " + relAge(now, back) + " ago"
	}
	labelWidth := w - 2 - 1 - len([]rune(tail))
	if labelWidth < trailMinLabel {
		return textStyle.Render(glyphBranch) + " " + dimStyle.Render(clip(tail, w-2))
	}
	return textStyle.Render(glyphBranch) + " " + dimStyle.Render(pad(clip(branchName(br.Label), labelWidth), labelWidth)) + " " + dimStyle.Render(tail)
}
