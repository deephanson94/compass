...
// swapPair is `l` in the pair (#400): the keys cross to the follower. The
// follower becomes the session being read — the selection, the header,
// `enter` and `r` with it, as #46 has them — and the pair re-opens from its
// side: a peer's pair is symmetric, so the old side becomes the follower;
// a lane's linked session pairs with nothing, so the keys read it alone
// and `h` brings the lane and its pair back (#396's place). The mark
// lands on the row the follower stood on — the `▹` the person was
// watching — not on the session's present, and neither half's events
// wait on a poll: each already has the other's.
func (m *Model) swapPair() {
	target := m.pairKey
	_, rw := m.pairWidths()
	doc := m.pairDoc(rw)
	row := m.pairRow(rw)
	...
	if row >= 0 && row < len(doc) {
		at, text = doc[row].at, readerRowText(doc, row)
	}
	oldEvents, pairEvents := m.events, m.pairEvents
	m.point(target)
	if m.level < levelReader {
		return
	}
	...
	if m.pairKey != "" {
		// The pair re-opened on the old side: its conversation is what
		// the keys just left, in hand already.
		m.pairEvents, m.pairPolled = oldEvents, true
		m.pairCache.valid = false
	}
	if !at.IsZero() {
		m.anchorOn(at, text, -1)
	}
}
