...
| `s` | on a session's legs, the summary: the legs counted by class with the time each took (`build 32 legs 3h45m`, `test 32 legs · 10 red`, the subagent lanes last), a class of one as its leg's own row. `space` opens a class into its legs, oldest first, one line each; `tab` on a leg is the trail with the cursor there; `s` or `esc` is the trail again |
